...
<Response>
  ...
  <Hangup/>
     ...
     */
    async handleStatusCallback(callSid, callStatus, duration) {
        try {
     ...
     */
    async handleIVRResponse(orderId, digits) {
        try {
            let responseType;
            let responseMessage;
            let success = false;
...
<Response>
  ...
  <Hangup/>
            ...
            const itemsList = order.orderItems.map(item => `${item.quantity} ${item.menuItem.name}`).join(', ');
            return `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Gather numDigits="1" action="/api/calls/handle-response/${orderId}" method="POST" timeout="30">
    <Say voice="alice">
